...
// ── Read Auth Sheet ───────────────────────────────────────────────────────────
async function readAuthUsers(): Promise<{ username: string; password: string; role: string }[]> {
  ...
  const userIdx = headers.findIndex((h: string) => h.includes("user") || h.includes("team") || h.includes("name"));
  const passIdx = headers.findIndex((h: string) => h.includes("pass") || h.includes("pwd") || h.includes("secret"));
  const roleIdx = headers.findIndex((h: string) => h.includes("role") || h.includes("type") || h.includes("access"));
  ...
  return rows
    .slice(1)
    .filter((row: any[]) => row[ui]?.toString().trim())
    .map((row: any[]) => ({
...
// ── Team Matching ─────────────────────────────────────────────────────────────
// Normalizes team names so all these are treated as the same:
//   "Content-OPS" = "Content OPS" = "Content Operations" = "Content Operation"
//   "Academics"   = "Academic"
// "shared" teams are isolated from their parent (Marketing ≠ Marketing-Shared).
function filterByTeam(username: string): (dataTeam: string) => boolean {
  const norm = (s: string) => s
    .toLowerCase()
    .replace(/[^a-z0-9]/g, " ")      // separators → space
    .replace(/\boperations?\b/g, "ops") // operation / operations → ops
    .replace(/\s+/g, "");             // collapse spaces

  const userNorm = norm(username);
  const userHasShared = userNorm.includes("shared");

  return (dataTeam: string) => {
    const dataNorm = norm(dataTeam);
    if (dataNorm === userNorm) return true;

    // Shared teams are isolated — never bleed into parent team and vice versa
    if (dataNorm.includes("shared") !== userHasShared) return false;

    // Prefix match (min 5 chars) covers:
    //   "Academics" ↔ "Academic", "Skills-Eng-ON" ↔ "Skills (Eng- ON)", etc.
...
// Public endpoint — returns team names extracted from the 4 budget sheets
app.get("/api/teams", async (req, res) => {
  try {
    if (teamsCache && Date.now() - teamsCache.cachedAt < 5 * 60 * 1000) {
    ...
    const skip = new Set(["other", "general", "department", "unrecognized", "unrecohnized", "team", "branch / group", "branch", "sub-team", "cost type", "source", "purpose / details"]);

    [...parsed.ltdQ2, ...parsed.lcQ2].forEach((i: any) => {
      const v = (i.department || "").trim();
      if (v.length >= 2 && !skip.has(v.toLowerCase())) teamSet.add(v);
    });
    [...parsed.ltdQ3, ...parsed.lcQ3].forEach((i: any) => {
      const v = (i.team || "").trim();
      if (v.length >= 2 && !skip.has(v.toLowerCase())) teamSet.add(v);
    });

    // Always include admin username from auth sheet
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    // Find each selected team in the auth sheet
    const matched = teams.map((t: string) =>
      allUsers.find(u => u.username.toLowerCase() === t.toLowerCase())
    );
    ...
    // All selected teams must share the same password
    ...
    const role: "admin" | "team" = matched.some(u => u!.role === "admin") ? "admin" : "team";
...
// ── Requisitions Sheet Helper ─────────────────────────────────────────────────
async function ensureRequisitionsSheet(sheets: any, spreadsheetId: string): Promise<void> {
  const meta = await sheets.spreadsheets.get({ spreadsheetId });
  const exists = (meta.data.sheets || []).some(
    (s: any) => s.properties?.title === "Requisitions"
  );
  if (exists) return;
  ...
  await sheets.spreadsheets.values.update({
    ...
    requestBody: {
      values: [[
        "Timestamp", "ID", "Team", "Submitted By", "Designation", "Contact", "Email",
        "Date of Submission", "Department", "Month", "Budget Line", "Cost Heading",
        "Amount (৳)", "Project Name", "Vendor Name", "Purpose", "Description",
        "Invoice Link", "Status",
      ]],
    },
  });
  ...
  try {
    ...
    const data = rows
      ...
      .map((row: any[]) => ({
        ...
        description: row[16] || "",
        invoiceLink: row[17] || "",
        status: (row[18] || "Pending") as "Pending" | "Approved" | "Rejected",
      }));
    ...
    const filtered =
      user.role === "admin"
        ? data
        : data.filter((r: any) =>
            r.team.split(",").map((s: string) => s.trim().toLowerCase()).some((t: string) => userTeamSet.has(t))
          );
...
app.post("/api/requisitions", requireAuth, async (req, res) => {
  ...
  const requiredFields = [
    "submittedBy", "designation", "contact", "email", "dateOfSubmission",
    "department", "month", "budgetLine", "costHeading", "vendorName",
    "purpose", "description", "invoiceLink",
  ];
  ...
  try {
    ...
    await sheets.spreadsheets.values.append({
      ...
      requestBody: {
        values: [[
          timestamp, id, teamStr,
          b.submittedBy, b.designation, b.contact, b.email, b.dateOfSubmission,
          b.department, b.month, b.budgetLine, b.costHeading,
          parseFloat(b.amount) || 0,
          b.projectName || "",
          b.vendorName, b.purpose, b.description, b.invoiceLink,
          "Pending",
        ]],
      },
    });
...
app.patch("/api/requisitions/:id", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Fetch column B (IDs) to locate the row
    ...
    // rowIndex is 0-based array index; sheet row is rowIndex + 1
    await sheets.spreadsheets.values.update({
      spreadsheetId,
...
app.get("/api/sheets-data", requireAuth, async (req, res) => {
  try {
    ...
    // Detect Q4 tabs (only include them in the batch if they exist in the sheet)
    ...
    // Q4 ranges are appended at index 4/5 only when those tabs exist
    let ltdQ4RawIdx = 4;
    ...
    // Filter data for non-admin users — show rows matching ANY of the selected teams.
    if (user.role !== "admin") {
      const matchers = user.teams.map(t => filterByTeam(t));
      const anyMatch = (dataTeam: string) => matchers.some(m => m(dataTeam));
      parsedLtdQ2 = parsedLtdQ2.filter((i: any) => anyMatch(i.department));
      parsedLtdQ3 = parsedLtdQ3.filter((i: any) => anyMatch(i.team));
      parsedLcQ2  = parsedLcQ2.filter((i: any) => anyMatch(i.department));
      parsedLcQ3  = parsedLcQ3.filter((i: any) => anyMatch(i.team));
      if (parsedLtdQ4) parsedLtdQ4 = parsedLtdQ4.filter((i: any) => anyMatch(i.team));
      if (parsedLcQ4)  parsedLcQ4  = parsedLcQ4.filter((i: any) => anyMatch(i.team));
    }
...
// ── Q2 Sheet Parser ───────────────────────────────────────────────────────────
function parseQ2Sheet(rows: any[][], entity: "LTD" | "LC") {
  if (rows.length < 2) return [];
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (rawDept && !rawHeading && !rawDesc) { currentGroup = rawDept; continue; }
    ...
    cleanedData.push({
      ...
      isHeader: false,
      aprBudget,
      mayBudget, mayActual, mayVariance, mayRemaining,
      junBudget, junActual, junVariance, junRemaining,
...
// ── Q3 Sheet Parser ───────────────────────────────────────────────────────────
function parseQ3Sheet(rows: any[][], entity: "LTD" | "LC") {
  ...
  for (let r = 0; r < Math.min(rows.length, 10); r++) {
    ...
    if (row && row.length > 5) {
      ...
      if (hasTeamOrBranch) { headerRowIndex = r; headers = row; break; }
    }
  }

  if (headers.length === 0) { headers = rows[2] || []; headerRowIndex = 2; }
  ...
  for (let i = headerRowIndex + 1; i < rows.length; i++) {
    ...
    if (rawTeam && !rawSubTeam && !rawCostType && !rawPurpose) { currentTeam = rawTeam; continue; }
    if (rawTeam) currentTeam = rawTeam;

    const isSubtotalFlag = (
      rawCostType.toLowerCase().includes("subtotal") || rawCostType.toLowerCase().includes("total") ||
      rawSubTeam.toLowerCase().includes("subtotal") || rawSubTeam.toLowerCase().includes("total") ||
      rawTeam.toLowerCase().includes("subtotal") || rawTeam.toLowerCase().includes("total")
    );
    ...
    cleanedData.push({
      ...
      purpose: rawPurpose,
      isSubtotal: isSubtotalFlag,
      julBudget, julActual, julVar, julRem,
      augBudget, augActual, augVar, augRem,
      sepBudget, sepActual, sepVar, sepRem,
...
// ── LC Q3 Parser ──────────────────────────────────────────────────────────────
function parseLcQ3Sheet(rows: any[][], entity: "LTD" | "LC") {
...
// On Vercel the app is exported as a serverless function — skip the local server
